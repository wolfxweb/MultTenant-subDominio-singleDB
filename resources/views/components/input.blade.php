...
<div class="form-floating {{ isset($classe) ? $classe : 'col-md-12' }} p-1" >
    <input name="{{ $wireNome }}" wire:model="{{ $wireNome }}" type="{{ isset($type) ? $type : 'text' }}"class="{{ isset($classeInput) ? $classeInput : 'form-control ' }} @error($wireNome) is-invalid @enderror " id="{{ $idInput }}" placeholder="{{ $label }} "  x-ref="{{$wireNome}}">
